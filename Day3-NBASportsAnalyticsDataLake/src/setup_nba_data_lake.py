...
# AWS configurations
region = "eu-west-2"  # eu-west-2 is London - Replace with your preferred AWS region
bucket_name = "blessing-sports-analytics-data-lake"  # Change to a unique S3 bucket name
...
if not api_key:
    raise ValueError("API key not found in the environment variables.")  # Ensure the API key is present

nba_endpoint = f"https://api.sportsdata.io/v3/nba/scores/json/Players?key={api_key}"  # Append API key to endpoint
